...
exports.searchBaltal = async(req, res, next) => {
    ...
    try {
        ...
        let storeList  =["secretGarden_RiverTown","secretGarden_Hyewha" ,"secretGarden_CenematicHyewha" ] 
        ...
        // return res.status(200).json(resultData);
        return res.status(200) .contentType('text/xml')
        .send(JSON.stringify(resultData.concat(resultData2).concat(resultData3)));
    }catch(err){
        return res.status(500).json(err);
    }        
}